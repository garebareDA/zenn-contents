...
# 原因
RailsにはAuto-Loadという機能があり`config.autoload_lib(ignore:)`を設定している場合`lib/`にそのまま配置してしまうと`db:seed`やその他Railsを実行する際に`NameError: uninitialized constant CustomCops::RuboCop`のようなNameErrorがでてしまいます。また開発環境のみでRuboCopを使っている場合も未定義になるのでproduction環境でNameErrorになります。
